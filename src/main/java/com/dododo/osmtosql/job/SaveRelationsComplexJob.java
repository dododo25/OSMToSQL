...
public class SaveRelationsComplexJob extends ComplexAbstractJob {

    private final PostgreSQLService service;

    private final Collection<Relation> relations;

    public SaveRelationsComplexJob(PostgreSQLService service, Collection<Relation> relations) {
    ...
    protected List<Callable<Integer>> prepareJobs() {
        ...
        Collection<Relation> filtered = relations.stream().filter(Relation::isComplete).collect(Collectors.toSet());

        Relation.SaveRelationMembersRequestBuilder saveMembersRequestBuilder = new Relation
                .SaveRelationMembersRequestBuilder(Constants.OSM_SCHEMA);
        Tag.SaveEntityTagRequestBuilder<Relation> saveTagsRequestBuilder = new Tag
                .SaveEntityTagRequestBuilder<>(Constants.OSM_SCHEMA, Constants.RELATION_TAGS_TABLE);

        jobs.add(new SaveRelationsJob(service, filtered));
